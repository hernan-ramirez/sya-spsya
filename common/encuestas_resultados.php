...
if (!isset( ${"cook_encuesta_sya_id".$voto_encuesta} )){ 
	if (isset($voto_encuesta)){
		$sql = "UPDATE encuestas_respuestas 
		SET votos = votos + 1 
		WHERE id = " . $voto_encuesta;
		mysql_query($sql);
		setcookie("cook_encuesta_sya_id".$voto_encuesta, date("Y-m-d H:i") ,time()+365*24*3600);
	}
}else{
	$mensaje = "Usted ya votó en esta encuesta";
}
?>
<HTML>
<HEAD>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <? if(isset($mensaje)){ ?>
  <tr> 
    <td align="center" valign="bottom" class="PopEncuestaTop"><B><FONT color="#FFFFFF"><? echo $mensaje ?></FONT></B></td>
  </tr>
  <? } ?>
  <tr> 
