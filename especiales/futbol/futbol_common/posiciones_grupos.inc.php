<?
$sql = "
SELECT grupo_primer_club AS grupo
FROM futbol_partidos
WHERE id_torneo = $id_torneo AND grupo_primer_club IS  NOT  NULL 
GROUP  BY grupo_primer_club

UNION

SELECT grupo_segundo_club AS grupo
FROM futbol_partidos
WHERE id_torneo = $id_torneo AND grupo_segundo_club IS  NOT  NULL 
GROUP  BY grupo_segundo_club
";
...
LEFT JOIN futbol_partidos p on (p.id_primer_club = c.id)
WHERE p.id_torneo = $id_torneo
AND p.grupo_primer_club = '$numero_grupo' 
AND p.orden_llave = 0
...
SELECT c.id, c.alias
FROM futbol_clubes c
LEFT JOIN futbol_partidos p on (p.id_segundo_club = c.id)
WHERE p.id_torneo = $id_torneo
AND p.grupo_segundo_club = '$numero_grupo'
AND p.orden_llave = 0
