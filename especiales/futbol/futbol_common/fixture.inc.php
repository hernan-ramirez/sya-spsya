...
<DIV id="fix" style="position:relative; width:100%; height:230px; overflow: auto;"> 
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
    ...
    <?
		...
		if($row["grupo"]!=$almecen_grupo && $row["grupo"]!="0"){	 
			?>
    <tr> 
      <TD colspan="5" class="BackEspecialesFst"><B>Grupo <? echo $row["grupo"] ?></B></TD>
    </tr>
    <?
			$almecen_grupo = $row["grupo"];
		}
		?>
    <TR> 
      <TD nowrap class="Terra<? echo $estilo ?>"> 
        <? if($row["estado_partido"]!=""){ echo $row["estado_partido"]; }else{ echo "Pendiente";}?>
      </TD>
      <TD width="50%" class="Green<? echo $estilo ?>"><? echo $row["equipo1"] ?>&nbsp;</TD>
      <TD align="center" nowrap class="Green3"><A href="javascript:abrirVentana('<? echo $common ?>ficha_partido.php?id_partido=<? echo $row["id"] ?>','<? echo $row["id"] ?>',503,350);"> 
